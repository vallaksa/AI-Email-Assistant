...
public class HttpAIProvider implements AIProvider {
    ...
    public HttpAIProvider(AIProviderProperties properties) {
        ...
        // Validate AI API URL configuration
    ...
    /**
     * Validates that AI provider configuration is properly set.
    ...
    private void validateConfiguration() {
        ...
        if (apiUrl == null || apiUrl.isBlank()) {
            throw new IllegalStateException(
                "AI_API_URL is not configured. Please set the AI_API_URL environment variable. " +
                "Example: export AI_API_URL=http://localhost:11434/api/generate"
            );
        }
        
        // Check if environment variable was not resolved (contains ${...})
        if (apiUrl.contains("${")) {
            throw new IllegalStateException(
                String.format(
                    "AI_API_URL environment variable is not set. Current value: '%s'. " +
                    "Please set the AI_API_URL environment variable. " +
                    "Example: export AI_API_URL=http://localhost:11434/api/generate",
                    apiUrl
                )
            );
        ...
        try {
            ...
            throw new IllegalStateException(
                String.format(
                    "AI_API_URL is not a valid URL: '%s'. Error: %s",
                    apiUrl,
                    e.getMessage()
                ),
                e
    ...
    public String generateReply(EmailMessage email, String userInstruction) {
        ...
        try {
            ...
            // Check if the error message already contains helpful information
            String message = e.getMessage();
            if (message != null && message.contains("AI service unavailable")) {
                throw new ProviderException(message, e);
            }
            throw new ProviderException(
                String.format("Failed to connect to AI service at %s. Please verify the service is running and accessible.", apiUrl),
                e
            );
        } catch (InterruptedException e) {
            log.error("Interrupted waiting for AI response");
            Thread.currentThread().interrupt();
            throw new ProviderException("AI response interrupted. Please try again.", e);
        } catch (IllegalStateException e) {
            // Configuration errors should be clear and actionable
            log.error("Configuration error: {}", e.getMessage());
            throw new ProviderException(e.getMessage(), e);
        } catch (IllegalArgumentException e) {
            // URI/URL format errors
            String message = e.getMessage();
            if (message != null && message.contains("Illegal character")) {
                throw new ProviderException(
                    String.format(
                        "AI_API_URL configuration is invalid. Please check your environment variables. " +
                        "Current value appears to be unresolved: '%s'. " +
                        "Set AI_API_URL environment variable. Example: export AI_API_URL=http://localhost:11434/api/generate",
                        apiUrl
                    ),
                    e
                );
            }
            throw new ProviderException("Invalid AI service configuration: " + e.getMessage(), e);
        } catch (Exception e) {
    ...
    private JsonNode callApi(String body) throws IOException, InterruptedException {
        ...
        if (response.statusCode() < 200 || response.statusCode() >= 300) {
            String errorMessage = String.format(
                "AI service unavailable (status %d). Please check if the AI service (e.g., Ollama) is running at %s",
                response.statusCode(),
                apiUrl
            );
            log.error("AI provider error: {} - Response body: {}", errorMessage, response.body());
